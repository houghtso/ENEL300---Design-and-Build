...
const int lpfScaleMax = 3;
const int hpfScaleMax = 4;
...
int scaled(int value, bool isLPF) {
  int min, max;
  if (isLPF) {
    min = scaleMin;
    max = lpfScaleMax;
  ...
  return map(value, 0, MAX_ANALOG_READ, min, max);
}
...
void failScreen(int number) {
  ...
  lcd.setCursor(5, 2);  // Move the cursor to the third row
  lcd.print("Score: ");
  lcd.print(number);
  lcd.print("/10");
  delay(600);
...
void loop() {
  ...
  if (currentTime - lastScaledTime >= 1000) {
    ...
    bool isLPF = (lpfMax != 0);
    scaledValue = scaled(currentValue, isLPF);
